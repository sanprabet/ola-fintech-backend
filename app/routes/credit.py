...
async def get_active_credit(uid: str, app_service: AppService = Depends(get_app_service))-> ApiResponse:
    """ Check if user has an ongoing credit, and return the credit data. """
    try:
        credit = await app_service.get_credit(uid)
...
async def request_credit(
    ...
    Returns:
        ApiResponse: The API response object, no data.
...
@router.put("/creditExtension/{uid}", response_model=ApiResponse)
async def request_credit_extension(
    ...
    """
    try:
        await app_service.request_extension_credit_payment(uid)
        return ApiResponse(
            data={},
            message="Tu extension de plazo fue solicitada exitosamente.",
            success=True
        )
